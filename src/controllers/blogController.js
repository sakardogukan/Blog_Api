...
module.exports.BlogPost = {
    list: async (req, res) => {
        ...
        //! RUN:
        // const data = await BlogPost.find(search).sort(sort).skip(skip).limit(limit).populate('blogCategoryId', 'userId')
        const data = await req.getModelList(BlogPost, ('blogCategoryId', 'userId'))
        res.status(200).send({
            error: false,
            count: data.length,
    ...
    listInPost: async (req, res) => {
        const data = await BlogPost.find({ userId: req.params.userId }).populate('userId')
        res.status(200).send({
            error: false,
            count: data.length,
            result: data
